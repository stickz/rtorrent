...
Manager::schedule(Window* w, rak::timer t) {
  rak::priority_queue_upsert(&m_scheduler, w->task_update(), t);
...
Manager::schedule_update(uint32_t minInterval) {
  ...
  if (!m_taskUpdate.is_queued() || m_taskUpdate.time() > m_scheduler.top()->time()) {
    rak::priority_queue_upsert(&taskScheduler, &m_taskUpdate, std::max(m_scheduler.top()->time(), m_timeLastUpdate + minInterval));
  }
}
